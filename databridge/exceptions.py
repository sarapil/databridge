...
"""
DataBridge — Custom Exception Hierarchy
"""
import frappe


class DataBridgeError(Exception):
    """DataBridge base exception."""
    def __init__(self, message=None, title=None):
        self.message = message or frappe._("An error occurred in DataBridge")
        self.title = title or frappe._("DataBridge Error")
        super().__init__(self.message)


class ValidationError(DataBridgeError):
...
class NotFoundError(DataBridgeError):
...
class PermissionError(DataBridgeError):
...
class ConfigurationError(DataBridgeError):
...
class IntegrationError(DataBridgeError):
